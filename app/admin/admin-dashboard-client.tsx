    ...
    try {
      ...
      if (systemRes?.success && systemRes?.status) {
        setSystemStatus({
          ...
          timestamp: systemRes.timestamp,
          checks: systemRes.checks || {},
          environment: systemRes.environment || {
            label: '—',
            nodeEnv: '',
            isVercel: false,
            vercelEnv: null,
          ...
          dbLatencyMs: systemRes.dbLatencyMs ?? null,
          mail: systemRes.mail,
        });
      }
      ...
      <main className="w-full px-1 sm:px-3 md:px-4 lg:px-6 py-2 sm:py-4 md:py-6 space-y-4 sm:space-y-6 md:space-y-8 lg:space-y-12 relative">
                    ...
                    <div className="flex items-center gap-3 sm:gap-4">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap items-center gap-2 mt-3">
                          ...
                          {systemStatus.mail && (
                            <span
                              className={`text-xs tabular-nums px-2 py-0.5 rounded-md border ${systemStatus.mail.configured ? 'border-emerald-500/30 text-emerald-800 dark:text-emerald-200 bg-emerald-500/10' : 'border-amber-500/40 text-amber-900 dark:text-amber-100 bg-amber-500/10'}`}
                              title="Sunucu ortamı (.env) — işlem e-postası"
                            >
                              E-posta:{' '}
                              {systemStatus.mail.configured
                                ? [systemStatus.mail.smtp && 'SMTP', systemStatus.mail.resend && 'Resend'].filter(Boolean).join(' + ') || '—'
                                : 'yapılandırılmadı'}
                            </span>
                          )}
                        </div>
        ...
        <section className="space-y-3 sm:space-y-4">
          ...
          <div className="grid gap-2 sm:gap-3 md:gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
            {analyticsData != null && (() => {
              ...
              return (
                <Link prefetch={false} href="/admin/analytics">
            ...
            {aiStats != null && (
              <Link prefetch={false} href="/admin/ai-dashboard">
        ...
        {cardStats && (
          <section className="space-y-4">
            ...
            <div className="flex justify-end"><Link prefetch={false} href="/admin/cards"><Button variant="outline" size="sm">Kartlar →</Button></Link></div>
          </section>
        )}
                ...
                <CardContent className="p-3 sm:p-4 md:p-5">
                  <div className="flex flex-wrap gap-3">
                    {segmentSummary.map((seg) => {
                      ...
                      return <Link prefetch={false} key={seg.id} href="/admin/segments"><Badge variant="secondary" className={`px-3 py-1.5 text-sm font-medium cursor-pointer hover:opacity-90 border-0 ${colorClass}`}><span className="mr-1.5">{seg.icon}</span>{seg.name} {seg.count}</Badge></Link>;
                    })}
                  </div>
                  <div className="mt-3 flex justify-end"><Link prefetch={false} href="/admin/segments"><Button variant="ghost" size="sm">Segmentler →</Button></Link></div>
                </CardContent>
              </Card>
            </section>
          )}
            ...
            <section className="space-y-3 sm:space-y-4 h-full">
              ...
              <Card className="h-full border border-border bg-card/80 dark:bg-card/90 backdrop-blur-sm shadow-lg">
                <CardContent className="p-3 sm:p-4 md:p-5 space-y-3 sm:space-y-4">
                  ...
                  <div className="flex justify-end"><Link prefetch={false} href="/admin/compliance"><Button variant="outline" size="sm">Uyumluluk →</Button></Link></div>
                </CardContent>
              </Card>
            </section>
          )}
        ...
        {auditRecent.length > 0 && (
          <section className="relative z-10 mt-8 sm:mt-10 md:mt-12 border-t border-border/50 pt-6 sm:pt-8 md:pt-10 space-y-4">
            ...
            <div className="flex justify-end"><Link prefetch={false} href="/admin/audit"><Button variant="outline" size="sm">Tüm denetim günlüğü →</Button></Link></div>
          </section>
        )}
