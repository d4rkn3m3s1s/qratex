  ...
  return (
    <div className="relative mx-auto max-w-6xl pb-16 pt-1">
      ...
      <div className="relative space-y-10">
        ...
        <div className="relative space-y-0 md:pl-8">
          <div className="absolute left-3 top-3 bottom-3 hidden w-px bg-gradient-to-b from-amber-500/50 via-border to-transparent md:block" aria-hidden />
          ...
          {STEPS.map((step, idx) => {
            ...
            return (
              <motion.div
                ...
                className={cn('relative mb-6 md:mb-10', flip ? 'md:ml-[18%]' : 'md:mr-[12%]')}
              >
                <div
                  className={cn(
                    'relative overflow-hidden rounded-xl border-2 border-border bg-background/95 p-5 shadow-sm transition-shadow duration-300 hover:shadow-md dark:bg-background/80',
                    flip ? 'border-l-amber-500/50 md:border-l-4' : 'border-r-amber-500/50 md:border-r-4'
                  )}
                >
                  <div className="flex flex-wrap items-start gap-4">
                    <span className="flex size-12 shrink-0 items-center justify-center rounded-lg border border-border bg-muted/60 font-mono text-sm font-bold text-amber-700 dark:text-amber-300">
                      {step.code}
                    </span>
                    ...
                    <div className="min-w-0 flex-1 space-y-2">
                      ...
                      <p className="text-sm leading-relaxed text-foreground/90">{t(`dealerExperienceGuide.block.${step.key}.body`)}</p>
                    </div>
                  </div>
                </div>
              </motion.div>
