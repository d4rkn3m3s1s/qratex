...
export async function GET(req: NextRequest) {
  ...
  const tasks = await prisma.companyTask.findMany({
    ...
    select: { weekKey: true, status: true },
    take: 5000,
  });

  const byWeek = new Map<string, { total: number; done: number }>();
  for (const k of keys) byWeek.set(k, { total: 0, done: 0 });
  ...
  const series = keys.map((k) => {
    ...
    return { weekKey: k, total: v.total, done: v.done, pct: v.total ? Math.round((v.done / v.total) * 100) : 0 };
  });

  return NextResponse.json({ success: true, series }, { headers: PRIVATE_NO_STORE_HEADERS });
}
