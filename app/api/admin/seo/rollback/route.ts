...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!seoSetting) return NextResponse.json({ error: 'SEO ayarları bulunamadı' }, { status: 404 });

    const log = await prisma.auditLog.findFirst({
      where: { id: auditLogId, entity: 'settings', entityId: seoSetting.id },
    });
    if (!log || !log.oldData || typeof log.oldData !== 'object') {
      return NextResponse.json({ error: 'Geçersiz veya bulunamadı audit kaydı' }, { status: 404 });
    }

    const oldPayload = log.oldData as unknown as SeoSettingsPayload;
    if (!oldPayload.global) return NextResponse.json({ error: 'Eski veri geçersiz' }, { status: 400 });
