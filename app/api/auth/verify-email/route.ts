...
export async function GET(request: NextRequest) {
  try {
    ...
    if (vt.expires < new Date()) {
      ...
      return NextResponse.json({ success: false, error: 'Doğrulama linkinin süresi dolmuş' }, { status: 400 });
    }
    ...
    return NextResponse.json({ success: false, error: 'Doğrulama yapılamadı' }, { status: 500 });
  }
}
