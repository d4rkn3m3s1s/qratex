/**
 * Patch NextResponse.json in route.ts under given API subdirs (e.g. staff, auth).
 * Usage: node scripts/patch-scope-private-headers.mjs staff auth
 */
import fs from 'fs';
...
function walk(dir) {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) {
      if (e.name.startsWith('[') && e.name.includes('nextauth')) continue;
      files.push(...walk(p));
...
for (const scope of scopes) {
  const root = path.join(projectRoot, 'app', 'api', scope);
  const all = walk(root);
